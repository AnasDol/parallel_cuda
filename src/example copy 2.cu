...
__device__ void new_variate(int* matrix, int n, int* var, int size, int min, int max, int true_max, int deep, int* result_var, int* min_sum, FILE* logfile);
...
__device__ void new_variate(int* matrix, int n, int* var, int size, int min, int max, int true_max, int deep, int* result_var, int* min_sum, FILE* logfile) {
    ...
    for (int i = min; i <= true_max - size + 1; i++) {

        if (deep < size) {
            var[deep] = i;
            new_variate(matrix, n, var, size, i + 1, max, true_max + 1, deep + 1, result_var, min_sum, logfile);
...
__global__ void variate(int* matrix, int n, int* var, int size, int min, int max, int deep, int* result_var, int* min_sum, FILE* logfile) {
    ...
    printf("iter 1\n");
    new_variate(matrix, n, var, size, 1, 1, 8, deep, result_var, min_sum, logfile);
    printf("iter 2\n");
    new_variate(matrix, n, var, size, 2, 3, 8, deep, result_var, min_sum, logfile);
    printf("iter 3\n");
    new_variate(matrix, n, var, size, 4, 8, 8, deep, result_var, min_sum, logfile);
